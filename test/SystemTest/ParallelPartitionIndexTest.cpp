...
TEST_CASE("Test parallel partition index", "[short]")
{
  int a = 0;
  ...
  auto ppIndex = it.getIndex("parallelPartition");
  cfg->edit("parallelWorkers", (int64_t) 3);
  ...
  auto ruTensors2 = ppIndex->searchTensor(as0, 2);
  std::cout << "query tensor from pp\n" << ruTensors2[0] << std::endl;
  ppIndex->deleteTensor(as0, 2);
  ppIndex->endHPC();
  REQUIRE(a == 0);
}
