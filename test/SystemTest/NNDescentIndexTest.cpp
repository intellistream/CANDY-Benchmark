...
TEST_CASE("Test nndescent index", "[short]") {
  int a = 0;
  ...
  auto nnDescentIndex = it.getIndex("nnDescent");
  cfg->edit("graphK", int64_t(2));
  cfg->edit("parallelWorkers", int64_t(12));
  cfg->edit("delta", 0.01);
  cfg->edit("rho", 1.0);
  nnDescentIndex->setConfig(cfg);
  nnDescentIndex->setFrozenLevel(1);

  nnDescentIndex->offlineBuild(ta);
  auto ruTensors2 = nnDescentIndex->searchTensor(as0, 2);
  std::cout << "query tensor from nndescent (offline build)" << std::endl;
  for (auto tensor : ruTensors2) {
    std::cout << tensor << std::endl;
  }

  nnDescentIndex->reset();
  nnDescentIndex->insertTensor(ta);
  ruTensors2 = nnDescentIndex->searchTensor(as0, 2);
  std::cout << "query tensor from nndescent (online update)" << std::endl;
  for (auto tensor : ruTensors2) {
    std::cout << tensor << std::endl;
  }
  REQUIRE(a == 0);
