...
class CANDYRetriever(BaseRetriever):
    ...
    def setConfig(self,cfg:Dict):
        ...
        cfg['vecDim'] = 768
        cfg['metricType']="IP"
        self.idxRania=rania.createIndex(idxTag)
        self.idxRania.setConfig(rania.dictToConfigMap(cfg))
    def retrieve(
            self,
            query: str,
        ...
        q0 = embedQ[0:1, :]
        ru =  self.idxRania.searchStringObject(q0, self.top_k)
        strList = ru[0]
    ...
    def insertContext(self, ctx: str):
        ...
        return self.idxRania.insertStringObject(embeddings_batch, list_of_strings)

    def deleteContext(self, ctx: str):
        embeddings_batch, list_of_strings = self.encodeContext(ctx)
        return self.idxRania.deleteStringObject( embeddings_batch, 1)

    def deleteQuery(self, qtx: str, k=1):
        embeddings_batch, list_of_strings = self.encodeQuery(qtx)
        return self.idxRania.deleteStringObject(embeddings_batch, k)
