...
/**************************************************************
 * NSW structure implementation
 **************************************************************/

    int NSW::nb_neighbors(int layer_no) const {
        return cum_nneighbor_per_level[layer_no + 1] -
               cum_nneighbor_per_level[layer_no];
    }

    void NSW::set_nb_neighbors(int level_no, int n) {
    ...
    int NSW::cum_nb_neighbors(int layer_no) const {
        return cum_nneighbor_per_level[layer_no];
    }

    void NSW::neighbor_range(idx_t no, int layer_no, size_t* begin, size_t* end)
    const {
        size_t o = offsets[no];
    ...
    NSW::NSW(int M) : rng(12345) {
        set_default_probas(M, 1.0 / log(M));
        offsets.push_back(0);
        M_ = M;
    }

    int NSW::random_level() {
    ...
    void NSW::set_default_probas(int M, float levelMult) {
        int nn = 0;
    ...
    void NSW::clear_neighbor_tables(int level) {
    ...
    void NSW::print_neighbor_stats(int level) const {
  ...
  reduction(+: tot_reciprocal) reduction(+: n_node)
    ...
    void NSW::fill_with_random_links(size_t n) {
    ...
    int NSW::prepare_level_tab(size_t n, bool preset_levels) {
 ...
 */
    ...
    namespace {
        ...
        using NodeDistCloser = NSW::NodeDistCloser;
...
/// remove neighbors from the list to make it smaller than max_size
        void shrink_neighbor_list(
            ...
            NSW::shrink_neighbor_list(qdis, resultSet, returnlist, max_size);
...
/// point. The own neighbor list is assumed to be locked.
    void NSW::add_links_starting_from(
...
/** Do a BFS on the candidates list */
        ...
        std::priority_queue<NSW::Node> search_from_candidate_unbounded(
                const NSW& vanama,
                const Node& node,
                DistanceComputer& qdis,
                int ef,
    ...
    void NSW::permute_entries(const idx_t* map) {
        // remap levels
 ...
 **************************************************************/
    ...
    void NSW::MinimaxHeap::push(storage_idx_t i, float v) {
        if (k == n) {
            if (v >= dis[0])
                return;
            if (ids[0] != -1) {
    ...
    float NSW::MinimaxHeap::max() const {
        return dis[0];
    }

    int NSW::MinimaxHeap::size() const {
        return nvalid;
    }

    void NSW::MinimaxHeap::clear() {
        nvalid = k = 0;
    }

#ifdef __AVX2__
    int NSW::MinimaxHeap::pop_min(float* vmin_out) {
    assert(k > 0);
    static_assert(
...
// baseline non-vectorized version
    ...
    int NSW::MinimaxHeap::count_below(float thresh) {
        int n_below = 0;
        for (int i = 0; i < k; i++) {
            if (dis[i] < thresh) {
                n_below++;
