...
struct Graph {
    ...
    Graph(node_t* data, int N, int K)
        ...
        printf("end adding from known graph");
    }
    ...
    // copy constructor
    Graph( Graph& g) : Graph(g.N, g.K) {
        memcpy(data, g.data, N * K * sizeof(node_t));
        for(int i=0; i<N*K; i++){
            g.data_vec.push_back(data[i]);
        }
    }
...
struct NSG {
    ...
    std::shared_ptr<nsg::Graph<idx_t>> final_graph; ///< NSG graph structure
