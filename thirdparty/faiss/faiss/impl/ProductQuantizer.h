...
struct ProductQuantizer : Quantizer {
    ...
    bool is_onlinePQ = false; /// if use online PQ to incrementally append
    bool is_trained = false;
    mutable std::vector<uint32_t> assigned_ntotal; /// a M*ksub matrix recording the assigned points of M*ksub centroids
    /// initialization
    enum train_type_t {
        Train_default,
    ...
    online_type_t online_type = Online_default;
    ...
    /// Layout: (M, ksub, dsub)
    mutable std::vector<float> centroids;
    ...
    void set_online(bool is_online) {
        is_onlinePQ = is_online;
        assigned_ntotal=std::vector<uint32_t>(M*ksub, 0);
        if(online_type==Online_window){
            window.window_data = new std::vector<float>(d*window.window_size);
        }
    }
    void set_trained(){
        is_trained = true;
        ...
        ;
    bool get_online() const{return is_onlinePQ;};
    void update_centroid(size_t m, size_t k, const float* x) const{
        assigned_ntotal[m*ksub+k]= assigned_ntotal[m*ksub+k]+1;
        if(is_trained) {
            const float *xsub = x + m * dsub;
            if(online_type==Online_default) {
                for (size_t d_curr = 0; d_curr < dsub; d_curr++) {
                    centroids[m * ksub * dsub + k * dsub + d_curr] +=
                            (xsub[d_curr] - centroids[m * ksub * dsub + k * dsub + d_curr]) /
                            (assigned_ntotal[m * ksub + k] * 1.0);
                }
            }
            if(online_type==Online_window){
                if (window.window_current < window.window_size) {
                    window.window_current += 1;
                    for (size_t d_curr = 0; d_curr < dsub; d_curr++) {
                        (*window.window_data)[(window.window_current - 1) * d + m * ksub + d_curr] = xsub[d_curr];
                        window.window_sum += xsub[d_curr];
                    }
                    for (size_t d_curr = 0; d_curr < dsub; d_curr++) {
                        centroids[m * ksub * dsub + k * dsub + d_curr] +=
                                (window.window_sum / window.window_current * 1.0 -
                                 centroids[m * ksub * dsub + k * dsub + d_curr]) /
                                ((assigned_ntotal)[m * ksub + k] * 1.0);
                    }
                } else {
                    for(size_t d_curr = 0; d_curr<dsub; d_curr++){
                        window.window_sum -= (*window.window_data)[(window.window_current-1)*d + m*ksub + d_curr];
                        (*window.window_data)[(window.window_current-1)*d + m*ksub + d_curr] = xsub[d_curr];
                        window.window_sum += (*window.window_data)[(window.window_current-1)*d + m*ksub + d_curr];
                    }
                    for(size_t d_curr=0; d_curr<dsub; d_curr++){
                        centroids[m*ksub*dsub+k*dsub+d_curr]+=(window.window_sum/window.window_current*1.0 - centroids[m*ksub*dsub+k*dsub+d_curr])/
                                                              (assigned_ntotal[m*ksub+k]*1.0);
                    }
                    window.window_ptr+=1;
    ...
    ProductQuantizer(
            ...
            size_t M,      /* number of subquantizers */
            size_t nbits,
            Index* assigned); /* number of bit per subvector index */
    ProductQuantizer();
