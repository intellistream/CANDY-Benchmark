...
ProductQuantizer::ProductQuantizer(size_t d, size_t M, size_t nbits, Index* assigned)
            : Quantizer(d, 0), M(M), nbits(nbits), assign_index(assigned) {
    set_derived_values();
}
...
void compute_code(const ProductQuantizer& pq, const float* x, uint8_t* code) {
    ...
    for (size_t m = 0; m < pq.M; m++) {
        ...
        uint64_t idxm = 0;
        if (pq.transposed_centroids.empty() || pq.is_onlinePQ) {
...
void ProductQuantizer::compute_codes(const float* x, uint8_t* codes, size_t n)
    ...
    if (dsub < 16 || is_onlinePQ) { // simple direct computation
