...
/**************************************************************
 * Vanama structure implementation
 **************************************************************/

int Vanama::nb_neighbors(int layer_no) const {
    return cum_nneighbor_per_level[layer_no + 1] -
            cum_nneighbor_per_level[layer_no];
}

void Vanama::set_nb_neighbors(int level_no, int n) {
...
int Vanama::cum_nb_neighbors(int layer_no) const {
    return cum_nneighbor_per_level[layer_no];
}

void Vanama::neighbor_range(idx_t no, int layer_no, size_t* begin, size_t* end)
        const {
    size_t o = offsets[no];
...
void Vanama::set_default_probas(int M, float levelMult) {
    int nn = 0;
...
void Vanama::clear_neighbor_tables(int level) {
...
void Vanama::print_neighbor_stats(int level) const {
...
void Vanama::fill_with_random_links(size_t n) {
    ...
    int Vanama::prepare_level_tab(size_t n, bool preset_levels) {
 ...
 */
void Vanama::shrink_neighbor_list(
...
using NodeDistCloser = Vanama::NodeDistCloser;
...
void add_link(
        ...
        int level) {
    size_t begin, end;
    vanama.neighbor_range(src, level, &begin, &end);
    if (vanama.neighbors[end - 1] == -1) {
        // there is enough room, find a slot to add it
        size_t i = end;
        while (i > begin) {
            if (vanama.neighbors[i - 1] != -1)
                break;
            i--;
        }
        vanama.neighbors[i] = dest;
        return;
    }
    ...
    while (resultSet.size()) {
        vanama.neighbors[i++] = resultSet.top().id;
        resultSet.pop();
    }
    // they may have shrunk more than just by 1 element
    while (i < end) {
        vanama.neighbors[i++] = -1;
    }
}
...
void search_neighbors_to_add(
    ...
    while (!candidates.empty()) {
        ...
        // loop over neighbors
        size_t begin, end;
        vanama.neighbor_range(currNode, level, &begin, &end);
        for (size_t i = begin; i < end; i++) {
            storage_idx_t nodeId = vanama.neighbors[i];
            if (nodeId < 0)
                break;
            if (vt.get(nodeId))
                continue;
            ...
            if (results.size() < vanama.efConstruction || results.top().d > dis) {
                ...
                if (results.size() > vanama.efConstruction) {
                    results.pop();
                }
            }
        }
...
void Vanama::add_links_starting_from(
...
int search_from_candidates(
    ...
    bool do_dis_check = params ? params->check_relative_distance
                               : vanama.check_relative_distance;
    int efSearch = params ? params->efSearch : vanama.efSearch;
...
std::priority_queue<Vanama::Node> search_from_candidate_unbounded(
        const Vanama& vanama,
        const Node& node,
        DistanceComputer& qdis,
        int ef,
    ...
    while (!candidates.empty()) {
        ...
        size_t begin, end;
        vanama.neighbor_range(v0, 0, &begin, &end);
...
void Vanama::search_level_0(
    ...
    if (search_type == 1) {
        ...
        for (int j = 0; j < nprobe; j++) {
            ...
            int candidates_size = std::max(vanama.efSearch, int(k));
            MinimaxHeap candidates(candidates_size);
        ...
        search_from_candidates(
                vanama, qdis, k, idxi, simi, candidates, vt, search_stats, 0);
    }
}

void Vanama::permute_entries(const idx_t* map) {
    // remap levels
...
void Vanama::MinimaxHeap::push(storage_idx_t i, float v) {
    if (k == n) {
        if (v >= dis[0])
            return;
        if (ids[0] != -1) {
...
float Vanama::MinimaxHeap::max() const {
    return dis[0];
}

int Vanama::MinimaxHeap::size() const {
    return nvalid;
}

void Vanama::MinimaxHeap::clear() {
    nvalid = k = 0;
}

#ifdef __AVX2__
int Vanama::MinimaxHeap::pop_min(float* vmin_out) {
    assert(k > 0);
    static_assert(
...
int Vanama::MinimaxHeap::count_below(float thresh) {
    int n_below = 0;
    for (int i = 0; i < k; i++) {
        if (dis[i] < thresh) {
            n_below++;
