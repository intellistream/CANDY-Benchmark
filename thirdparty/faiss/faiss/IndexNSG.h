...
struct IndexNSG : Index {
    ...
    char build_type = 0;
    /// indicate how to incrementally append to a knn graph
    /// - 0: batch build a clustering graph then append to the original graph
    /// - 1: batch build a interconnected graph then append to the original graph
    char add_type = 1;
    ...
    void add_stream(idx_t n, const float* x) ;
    ...
    void check_knn_graph_stream(const idx_t* knn_graph, idx_t n, int K) const;
    void check_knn_graph_global(const idx_t* knn_graph, idx_t n, int K) const;
};
