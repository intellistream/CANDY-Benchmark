...
IndexLSH::IndexLSH(idx_t d, int nbits, bool rotate_data, bool train_thresholds)
    ...
    IndexLSH::IndexLSH(idx_t d, int nbits, MetricType metric_type, bool rotate_data, bool train_thresholds)
            : IndexFlatCodes((nbits + 7) / 8, d, metric_type),
              nbits(nbits),
              rotate_data(rotate_data),
              train_thresholds(train_thresholds),
              rrot(d, nbits) {
        is_trained = !train_thresholds;
        printf("METRIC_TYPE=%d\n", metric_type);
        if (rotate_data) {
            rrot.init(5);
        } else {
            FAISS_THROW_IF_NOT(d >= nbits);
