...
using MinimaxHeap = Vanama::MinimaxHeap;
using storage_idx_t = Vanama::storage_idx_t;
using NodeDistFarther = Vanama::NodeDistFarther;
...
void vanama_add_vertices(
    ...
    Vanama& vanama = index_vanama.vanama;
    size_t ntotal = n0 + n;
...
/**************************************************************
 * IndexVanama implementation
 **************************************************************/

IndexVanama::IndexVanama(int d, int M, MetricType metric)
        : Index(d, metric), vanama(M) {}

IndexVanama::IndexVanama(Index* storage, int M)
        : Index(storage->d, storage->metric_type), vanama(M), storage(storage) {}

IndexVanama::~IndexVanama() {
    if (own_fields) {
        delete storage;
    }
}

void IndexVanama::train(idx_t n, const float* x) {
    FAISS_THROW_IF_NOT_MSG(
            storage,
            "Please use IndexVanamaFlat (or variants) instead of IndexVanama directly");
    // vanama structure does not require training
    storage->train(n, x);
    is_trained = true;
}

void IndexVanama::search(
        idx_t n,
        const float* x,
        idx_t k,
        float* distances,
...
void IndexVanama::reconstruct(idx_t key, float* recons) const {
...
void IndexVanama::search_level_0(
        idx_t n,
        const float* x,
        idx_t k,
...
void IndexVanama::init_level_0_from_knngraph(
        int k,
        const float* D,
        const idx_t* I) {
...
void IndexVanama::init_level_0_from_entry_points(
        int n,
...
void IndexVanama::link_singletons() {
...
void IndexVanama::permute_entries(const idx_t* perm) {
...
/**************************************************************
 * IndexVanamaFlat implementation
 **************************************************************/



/**************************************************************
 * IndexVanamaPQ implementation
 **************************************************************/

IndexVanamaPQ::IndexVanamaPQ() = default;

IndexVanamaPQ::IndexVanamaPQ(int d, int pq_m, int M, int pq_nbits)
        : IndexVanama(new IndexPQ(d, pq_m, pq_nbits), M) {
    own_fields = true;
    is_trained = false;
}

void IndexVanamaPQ::train(idx_t n, const float* x) {
    IndexVanama::train(n, x);
    (dynamic_cast<IndexPQ*>(storage))->pq.compute_sdc_table();
}

/**************************************************************
 * IndexVanamaSQ implementation
 **************************************************************/

IndexVanamaSQ::IndexVanamaSQ(
        int d,
        ScalarQuantizer::QuantizerType qtype,
        int M,
        MetricType metric)
        : IndexVanama(new IndexScalarQuantizer(d, qtype, metric), M) {
    is_trained = this->storage->is_trained;
    own_fields = true;
}

IndexVanamaSQ::IndexVanamaSQ() = default;

/**************************************************************
 * IndexVanama2Level implementation
 **************************************************************/

IndexVanama2Level::IndexVanama2Level(
        Index* quantizer,
        size_t nlist,
        int m_pq,
        int M)
        : IndexVanama(new Index2Layer(quantizer, nlist, m_pq), M) {
    own_fields = true;
    is_trained = false;
}

IndexVanama2Level::IndexVanama2Level() = default;
...
void IndexVanama2Level::search(
    ...
    if (dynamic_cast<const Index2Layer*>(storage)) {
        IndexVanama::search(n, x, k, distances, labels);
...
void IndexVanama2Level::flip_to_ivf() {
