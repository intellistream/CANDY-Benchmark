...
IndexPQ::IndexPQ(bool is_online,int d, size_t M, size_t nbits, MetricType metric): IndexFlatCodes(0, d, metric), pq(d, M, nbits) {
    is_trained = false;
    do_polysemous_training = false;
    polysemous_ht = nbits * M + 1;
    search_type = ST_PQ;
    ...
    if(is_online){
        printf("Creating M*ksub records of assigned points!\n");
        pq.set_online(is_online);
    }
}
