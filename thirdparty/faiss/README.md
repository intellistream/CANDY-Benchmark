...
## Install on Ubuntu
```shell
sudo apt install *lapck* *blas*
mkdir build && cd build
cmake .. -DFAISS_ENABLE_GPU=OFF -DFAISS_ENABLE_PYTHON=OFF -DENABLE_PAPI=ON -DREBUILD_PAPI=ON
pip install pandas matplotlib numpy
make
```
## Hacked parts
1. enable to build the tutorial program at <build folder>/tutorial/cpp
2. scripts at <build folder>/tutorial/scripts
```shell
cd <build folder>
./rerunAll.sh
```
