...
main(int argc, char **argv) {
  ...
  if (!TESTS_QUIET) {
    ...
    printf("available_domains: %#x (%s)\n",
           cmpinfo->available_domains,
           stringify_all_domains(cmpinfo->available_domains));    /* Available domains */
    ...
    printf("available_granularities: %#x (%s)\n",
           cmpinfo->available_granularities,
           stringify_all_granularities(cmpinfo->available_granularities));    /* Available granularities */
    printf("hardware_intr_sig: %d\n", cmpinfo->hardware_intr_sig);
