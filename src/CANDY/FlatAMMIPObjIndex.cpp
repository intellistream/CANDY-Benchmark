...
torch::Tensor CANDY::FlatAMMIPObjIndex::myMMInline(torch::Tensor &a, torch::Tensor &b, int64_t ss) {
  switch (ammType) {
...
void CANDY::FlatAMMIPObjIndex::reset() {
  lastNNZ = -1;
}
bool CANDY::FlatAMMIPObjIndex::insertTensor(torch::Tensor &t) {
  return INTELLI::IntelliTensorOP::appendRowsBufferMode(&dbTensor, &t, &lastNNZ, expandStep);
}

bool CANDY::FlatAMMIPObjIndex::insertStringObject(torch::Tensor &t, std::vector<std::string> &strs) {
...
bool CANDY::FlatAMMIPObjIndex::deleteTensor(torch::Tensor &t, int64_t k) {
...
bool CANDY::FlatAMMIPObjIndex::deleteStringObject(torch::Tensor &t, int64_t k) {
  std::vector<faiss::idx_t> idxToDelete = searchIndex(t, k);
  std::vector<int64_t> &int64Vector = reinterpret_cast<std::vector<int64_t> &>(idxToDelete);
...
bool CANDY::FlatAMMIPObjIndex::reviseTensor(torch::Tensor &t, torch::Tensor &w) {
...
std::vector<faiss::idx_t> CANDY::FlatAMMIPObjIndex::knnInline(torch::Tensor &query,
                                                              int64_t k,
...
std::vector<faiss::idx_t> CANDY::FlatAMMIPObjIndex::searchIndex(torch::Tensor q, int64_t k) {
...
std::vector<torch::Tensor> CANDY::FlatAMMIPObjIndex::getTensorByIndex(std::vector<faiss::idx_t> &idx, int64_t k) {
...
torch::Tensor CANDY::FlatAMMIPObjIndex::rawData() {
  return dbTensor.slice(0, 0, lastNNZ + 1).contiguous();
}

std::vector<torch::Tensor> CANDY::FlatAMMIPObjIndex::searchTensor(torch::Tensor &q, int64_t k) {
  auto idx = searchIndex(q, k);
  return getTensorByIndex(idx, k);
}

std::vector<std::vector<std::string>> CANDY::FlatAMMIPObjIndex::searchStringObject(torch::Tensor &q, int64_t k) {
