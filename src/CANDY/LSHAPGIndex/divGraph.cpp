...
{
	N=prep->data.N;
	//lastN = prep->data.lastStep;
	lastN = 0;
	linkLists.resize(N, nullptr);
	int unitL = max(efC, maxT);
	//printf("last N: %d this N:%d\n", lastN, N);
	...
	std::shuffle(idx, idx + N-lastN, std::default_random_engine(seed));
	first_id = idx[0];

	insertLSHRefine(idx[0]);//Ensure there is at least one point in the graph before parallelizing
	//lsh::progress_display pd(N - lastN);
//#pragma omp parallel for
	for (int i = 1; i < N-lastN; i++) {
