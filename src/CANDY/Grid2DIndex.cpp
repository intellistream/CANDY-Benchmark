...
#include <algorithm>
#include <utility>
bool CANDY::Grid2DIndex::setConfig(INTELLI::ConfigMapPtr cfg) {
  ...
  mlFitter.init(vecDim,2,cfg);
  gridStore.init(numberOfGrids);
  dmBuffer.init(vecDim, memBufferSize, 0, 0, 0);
  return true;
}
bool CANDY::Grid2DIndex::startHPC() {

  return true;
}
bool CANDY::Grid2DIndex::endHPC() {
  //dmBuffer.clear();
  return true;
}
void CANDY::Grid2DIndex::reset() {

}
bool CANDY::Grid2DIndex::insertTensor(torch::Tensor &t) {
 /* if (t.size(0) > memBufferSize && memBufferSize > 0) {
  ...
  return insertTensor2Grid(t);

}

bool CANDY::Grid2DIndex::deleteTensor(torch::Tensor &t, int64_t k) {
...
std::vector<int64_t> CANDY::Grid2DIndex::findTopKClosest(const torch::Tensor &query,
                                                             int64_t top_k,
                                                             int64_t batch_size
) {
...
bool CANDY::Grid2DIndex::resetIndexStatistics() {
  dmBuffer.clearStatistics();
  gpuComputingUs = 0;
  gpuCommunicationUs = 0;
  return true;
}
INTELLI::ConfigMapPtr CANDY::Grid2DIndex::getIndexStatistics() {
...
torch::Tensor CANDY::Grid2DIndex::distanceIP(torch::Tensor db,
                                                 torch::Tensor query,
                                                 int64_t cudaDev,
                                                 Grid2DIndex *idx) {
...
torch::Tensor CANDY::Grid2DIndex::distanceL2(torch::Tensor db0,
                                                 torch::Tensor _q,
                                                 int64_t cudaDev,
                                                 Grid2DIndex *idx) {
...
bool CANDY::Grid2DIndex::loadInitialTensor(torch::Tensor &t) {
  mlFitter.trainModel(t,1);
  insertTensor(t);
  return true;
}
bool CANDY::Grid2DIndex::insertTensor2Grid(torch::Tensor &t) {
  int64_t total_vectors = t.size(0);
  int64_t batch_size = DCOBatchSize;
  //torch::Tensor transposed_query = query.t();
  for (int64_t startPos = 0; startPos < total_vectors; startPos += batch_size) {
    int64_t endPos = std::min(startPos + batch_size, total_vectors);
     ...
     */
    int64_t positionStart = dmBuffer.size();
    dmBuffer.appendTensor(dbBatch);
    int64_t positionEnd = dmBuffer.size();
    /**
     * @brief add to grid
     */
    for(int64_t i=positionStart;i<positionEnd;i++) {
      int64_t codeI = i-positionStart;
      auto codeITensor = decodeBatch[codeI];
      int64_t x = codeITensor[0].item<int64_t>();
      int64_t y = codeITensor[1].item<int64_t>();
      int64_t item = i;
      gridStore.insertItemToGrid(x,y,item);
