...
torch::Tensor CANDY::YinYangGraphIndex::crsAmm(torch::Tensor &A,
                                               torch::Tensor &B,
...
torch::Tensor CANDY::YinYangGraphIndex::randomProjection(torch::Tensor &a) {
  if (useCRS) {
    if (redoCRSIndices) {
      genCrsIndices();
    }
    return CANDY::YinYangGraphIndex::crsAmm(a, rotationMatrix, crsIndices);
  }
...
bool CANDY::YinYangGraphIndex::setConfig(INTELLI::ConfigMapPtr cfg) {
  ...
  numberOfBuckets = cfg->tryI64("numberOfBuckets", 4096, true);
  ...
  INTELLI_INFO(
      "#YinYangGraph, #Buckets=" + std::to_string(numberOfBuckets) + ",encode length=" + std::to_string(encodeLen)
          + "sampling "
          + std::to_string(bucketsLog2) + " bits at first tier, max connections=" + std::to_string(maxConnection));
  //creat the buckets
  yyg.init(numberOfBuckets, encodeLen, maxConnection);
  /**
...
std::vector<uint8_t> CANDY::YinYangGraphIndex::encodeSingleRow(torch::Tensor &tensor, uint64_t *bucket) {
...
bool CANDY::YinYangGraphIndex::insertTensor(torch::Tensor &t) {
  ...
  for (int64_t i = 0; i < rows; i++) {
    ...
    yyg.insertTensorWithEncode(rowI, rowICode, (uint64_t) bkt);
  }
  return true;
}
...
std::vector<torch::Tensor> CANDY::YinYangGraphIndex::searchTensor(torch::Tensor &q, int64_t k) {
  ...
  for (int64_t i = 0; i < rows; i++) {
    ...
    auto candidateTensor =
        yyg.getMinimumNumOfTensors(rowI, rowICode, (uint64_t) bkt, k * candidateTimes);
