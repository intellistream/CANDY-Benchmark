...
#include "CANDY/SPFresh/MetadataSet.h"
...
#include "CANDY/SPFresh/Helper/LockFree.h"
...
MemMetadataSet::GetMetadataCopy(SizeType p_vectorID) const
{
    ...
    std::uint64_t startoff = m_offsets[p_vectorID];
    std::uint64_t bytes = m_offsets[p_vectorID + 1] - startoff;
    if (p_vectorID >= m_count || p_vectorID + 1 >= m_offsets.size()) {
        throw std::out_of_range("p_vectorID out of range.");
    }
    ...
    if (p_vectorID < m_count) {
        ...
        if (b.Data() == nullptr) {
            throw std::runtime_error("Memory allocation for ByteArray failed.");
        }
