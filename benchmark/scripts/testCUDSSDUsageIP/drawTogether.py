...
def runPeriod(exePath, algoTag, resultPath, configTemplate="config.csv", prefixTagRaw="null"):
    ...
    if (algoTag == 'CPU'):
        ...
        editConfig(exePath + "temp2.csv", exePath + "temp1.csv", "SSDBufferSize", 150000)
    if (algoTag == 'CPU-GPU'):
        editConfig(exePath + "temp1.csv", exePath + "temp2.csv", "cudaDevice", 1)
        editConfig(exePath + "temp2.csv", exePath + "temp1.csv", "SSDBufferSize", 150000)
    if (algoTag == 'CPU-SSD'):
    ...
    if (algoTag == 'nnDescent2 '):
        os.system("cp dummy.csv " + exePath + "onlineCUD_result.csv")
    else:
...
def readResultSingle(singleValue, resultPath):
    print(singleValue)
    resultFname = resultPath + "/" + str(singleValue) + "/onlineCUD_result.csv"
    ...
    incrementalDel = readConfig(resultFname, "95%latency(Del)")
    l3Stall = 0
    totalStall = 0
    froErr = 0
    return elapsedTime, incrementalBuild, incrementalSearch, recall, pendingWaitTime, incrementalDel, l3Stall, totalStall, froErr
...
def readResultVector(singleValueVec, resultPath):
    ...
    for i in singleValueVec:
        elapsedTime, incrementalBuild, incrementalSearch, recall, pendingWaitTime, incrementalDel, l3Stall, totalStall, fro = readResultSingle(
            i, resultPath)
        ...
        incrementalDelVec.append(float(incrementalDel))
    ...
    return np.array(elapseTimeVec), np.array(incrementalBuildVec), np.array(incrementalSearchVec), np.array(
        recallVec), np.array(
        pendingWaitTimeVec), np.array(incrementalDelVec), np.array(l3StallVec), np.array(totalStallVec), np.array(froVec)
...
def compareMethod(exeSpace, commonPathBase, resultPaths, csvTemplate, algos, dataSetName, reRun=1):
    ...
    for i in range(len(algos)):
        ...
        # exit()
        if resultIsComplete:
            elapsedTime, incrementalBuild, incrementalSearch, recall, pendingWaitTime, incrementalDel, l3Stall, totalStall, froVec = readResultVector(
            ...
            incrementalDelAll.append(incrementalDel)
    ...
    return np.array(elapsedTimeAll), np.array(incrementalBuildAll), np.array(periodAll), np.array(recallAll), np.array(
        incrementalSearchAll), np.array(pendingWaitTimeAll), np.array(incrementalDelAll), np.array(l3StallAll), np.array(
...
def main():
    ...
    commonBasePath = os.path.abspath(os.path.join(os.getcwd(), "../..")) + "/results/testCUDSSDUsageIP/"

    figPath = os.path.abspath(os.path.join(os.getcwd(), "../..")) + "/figures/testCUDSSDUsageIP"
    ...
    print(reRun)
    methodTags = algoDisp
    elapsedTimeAll, incrementalBuildAll, periodAll, recall, incrementalSearchAll, pendingWaitTimeAll, incrementalDelAll, l3StallAll, totalStallAll, froAll = compareMethod(
    ...
    df.to_csv(figPath + "/e2e_query_latency.csv", float_format='%.2f')
    df = pd.DataFrame((incrementalDelAll) / 1e6, columns=dataSetNames, index=algoDisp)
    df.to_csv(figPath + "/e2e_del_latency.csv", float_format='%.2f')
