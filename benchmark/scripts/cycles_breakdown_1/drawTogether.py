...
def readResultVector(singleValueVec, resultPath):
    ...
    for i in singleValueVec:
        elapsedTime, cpuCycle, memStall, instructions, l1dStall, l2Stall, l3Stall, totalStall = readResultSingle(i,
                                                                                                                 resultPath)
...
def main():
    ...
    for valueChose in range(len(valueVec)):
        ...
        accuBar.DrawFigure(methodTags,
                           [memStallPerMethod, l1dStallPerMethod, l2StallPerMethod, l3StallPerMethod,
                            otherPerMethod, nonStallPerMethod] / cpuCyclePerMethod * 100.0,
                           ['Mem Stall', 'L1D Stall', 'L2 Stall', 'L3 Stall', 'Other Stall', 'Not Stall'], '',
    ...
    groupBar2.DrawFigure(dataSetNames, totalStallAll / cpuCycleAll * 100.0, methodTags, "Datasets",
                         "Ratio of Stalls (%)", 5, 15, figPath + "stall_ratio", True)
    groupBar2.DrawFigure(dataSetNames, l1dStallAll / cpuCycleAll * 100.0, methodTags, "Datasets",
                         "Ratio of l1dStalls (%)", 5, 15, figPath + "l1dstall_ratio", True)
    groupBar2.DrawFigure(dataSetNames, l2StallAll / cpuCycleAll * 100.0, methodTags, "Datasets",
                         "Ratio of l2Stalls (%)", 5, 15, figPath + "l2stall_ratio", True)
    groupBar2.DrawFigure(dataSetNames, l3StallAll / cpuCycleAll * 100.0, methodTags, "Datasets",
                         "Ratio of l3Stalls (%)", 5, 15, figPath + "l3stall_ratio", True)
    groupBar2.DrawFigureYLog(dataSetNames, memStallAll, methodTags, "Datasets", "Count of memory stall", 5, 15,
                             figPath + "cnt_mem_stall", True)
    print(ipcAll)
