...
from genConceptDriftTopic import genConceptDriftTopicEmbeddings
...
def prepareEmbeddings(commonBasePath, aRowVec):
    ...
    for i in range(len(aRowVec)):
        ...
        if (os.path.exists(desiredDataFname) and os.path.exists(desiredQueryFname)):
            ...
            genConceptDriftTopicEmbeddings(dataMax - int(aRowVec[i]), int(aRowVec[i]),
...
def runPeriod(exePath, algoTag, resultPath, configTemplate="config.csv", prefixTagRaw="null"):
    ...
    dataPathCommon = exePath + "/results/scanIPConceptDriftTopic/"
...
def main():
    ...
    commonBasePath = os.path.abspath(os.path.join(os.getcwd(), "../..")) + "/results/scanIPConceptDriftTopicNoDropping/"

    figPath = os.path.abspath(os.path.join(os.getcwd(), "../..")) + "/figures/scanIPConceptDriftTopicNoDropping"
    ...
    aRowVec = [100, 1000, 10000, 25000, 50000, 60000, 70000, 80000, 90000]

    # exit()
    # aRowVec=[100, 200]
    # add the algo tag here
    # algosVec = ['flat', 'LSH-H','flatAMMIP','flatAMMIPSMPPCA','PQ','IVFPQ','HNSW']
    algosVec = ['flat', 'LSH-H', 'flatAMMIP', 'flatAMMIPSMPPCA', 'PQ', 'IVFPQ', 'onlinePQ', 'HNSW', 'NSW', 'NSG',
                'nnDescent']
    # algosVec = ['flat', 'LSH-H']
    ...
    algoDisp = ['BrutalForce', 'LSH-H', 'AMM(CRS)', 'AMM(PCA)', 'PQ', 'IVFPQ', 'onlinePQ', 'HNSW', 'NSW', 'NSG',
                "NNDescent"]
