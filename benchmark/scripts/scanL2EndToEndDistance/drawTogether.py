...
def runPeriod(exePath, algoTag, resultPath, configTemplate="config.csv", prefixTagRaw="null"):
    ...
    if (algoTag == 'LSH-H'):
        ...
        editConfig(exePath + "temp4.csv", exePath + "temp1.csv", "encodeLen", 3)
    if (algoTag == 'LSH-ML'):
        editConfig(exePath + "temp1.csv", exePath + "temp2.csv", "congestionDropWorker_algoTag", "onlineIVFL2H")
        editConfig(exePath + "temp2.csv", exePath + "temp4.csv", "useCRS", 0)
        editConfig(exePath + "temp4.csv", exePath + "temp1.csv", "encodeLen", 3)
    ...
    if (algoTag == 'onlinePQ'):
        ...
        editConfig(exePath + "temp2.csv", exePath + "temp1.csv", "sketchSize", 256)
    if (algoTag == 'HNSW'):
        editConfig(exePath + "temp1.csv", exePath + "temp2.csv", "congestionDropWorker_algoTag", "HNSWNaive")
        editConfig(exePath + "temp2.csv", exePath + "temp1.csv", "opt_mode", 0)
    if (algoTag == 'HNSW-LAQ'):
        editConfig(exePath + "temp1.csv", exePath + "temp2.csv", "congestionDropWorker_algoTag", "HNSWNaive")
        editConfig(exePath + "temp2.csv", exePath + "temp1.csv", "opt_mode", 1)
    if (algoTag == 'HNSW-DCO'):
        editConfig(exePath + "temp1.csv", exePath + "temp2.csv", "congestionDropWorker_algoTag", "HNSWNaive")
        editConfig(exePath + "temp2.csv", exePath + "temp1.csv", "opt_mode", 2)
    exeTag = "onlineInsert"
    # prepare new file
...
def main():
    ...
    commonBasePath = os.path.abspath(os.path.join(os.getcwd(), "../..")) + "/results/scanL2E2EDISTANCE/"

    figPath = os.path.abspath(os.path.join(os.getcwd(), "../..")) + "/figures/scanL2E2EDISTANCE"
    ...
    algosVec = ['flat', 'HNSW', 'HNSW-LAQ','HNSW-DCO']
    # algosVec = ['flat', 'LSH-H']
    # algosVec = ['flat', 'onlinePQ']
    # algosVec=['incrementalRaw']
    # algosVec=[ 'pq']
    # algoDisp = ['BrutalForce', 'PQ']
    algoDisp = ['Baseline', 'HNSW', 'HNSW-LAQ','HNSW-DCO']
