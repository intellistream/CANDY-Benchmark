...
def readResultVector(singleValueVec, resultPath):
    ...
    for i in singleValueVec:
        elapsedTime, incrementalBuild, incrementalSearch, recall, l1dStall, l2Stall, l3Stall, totalStall, fro = readResultSingle(
            i, resultPath)
...
def compareMethod(exeSpace, commonPathBase, resultPaths, csvTemplate, algos, dataSetName, reRun=1):
    ...
    for i in range(len(algos)):
        ...
        # exit()
        if resultIsComplete:
            elapsedTime, incrementalBuild, incrementalSearch, recall, l1dStall, l2Stall, l3Stall, totalStall, froVec = readResultVector(
    ...
    return np.array(elapsedTimeAll), np.array(incrementalBuildAll), np.array(periodAll), np.array(recallAll), np.array(
        incrementalSearchAll), np.array(l1dStallAll), np.array(l2StallAll), np.array(l3StallAll), np.array(
...
def main():
    ...
    commonBasePath = os.path.abspath(
        os.path.join(os.getcwd(), "../..")) + "/results/scanIPEventRateFaissHalfWayDPR100KC4/"

    figPath = os.path.abspath(os.path.join(os.getcwd(), "../..")) + "/figures/scanIPEventRateFaissHalfWayDPR100KC4"
    ...
    aRowVec = [1000, 2000, 4000, 5000, 6000, 7000]
    ...
    print(reRun)
    methodTags = algoDisp
    elapsedTimeAll, incrementalBuildAll, periodAll, recall, incrementalSearchAll, l1dStallAll, l2StallAll, l3StallAll, totalStallAll, froAll = compareMethod(
    ...
    groupLine.DrawFigureYnormal(periodAll, incrementalBuildAll / 1000,
                                methodTags,
                                "Input Rate (line/s)", r'95% Latency of insert (ms)', 0, 1,
                                figPath + "/" + "scanIPEventRateFaissHalf_lat_INSERT",
                                True)
    groupLine.DrawFigureYnormal(periodAll, incrementalSearchAll / 1000,
                                methodTags,
                                "Input Rate (line/s)", r'Latency of search (ms)', 0, 1,
                                figPath + "/" + "scanIPEventRateFaissHalf_lat_search",
                                True)
    groupLine.DrawFigureYnormal(periodAll, recall,
                                methodTags,
                                "Input Rate (line/s)", r'recall@10', 0, 1,
                                figPath + "/" + "scanIPEventRateFaissHalf_recall",
                                True)
