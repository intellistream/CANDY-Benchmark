...
class CANDYRetriever(BaseRetriever):
    def __init__(
            ...
            custom_candy_name: Optional[str] = 'aknn0',
        ...
        self.candy_name = custom_candy_name
        # gen the input tensor
        torch.ops.CANDY.index_create(self.candy_name, 'flatAMMIPObj')
        torch.ops.CANDY.index_editCfgI64(self.candy_name, 'vecDim', 768)
        torch.ops.CANDY.index_init(self.candy_name)
    ...
    def retrieve(
        ...
        q0 = embedQ[0:1, :]
        ru = torch.ops.CANDY.index_searchString(self.candy_name, q0, self.top_k)
        strList = ru[0]
    ...
    def insertContext(self, ctx: str):
        ...
        return torch.ops.CANDY.index_insertString(self.candy_name, embeddings_batch, list_of_strings)

    def deleteContext(self, ctx: str):
        embeddings_batch, list_of_strings = self.encodeContext(ctx)
        return torch.ops.CANDY.index_deleteString(self.candy_name, embeddings_batch, 1)

    def deleteQuery(self, qtx: str, k=1):
        embeddings_batch, list_of_strings = self.encodeQuery(qtx)
        return torch.ops.CANDY.index_deleteString(self.candy_name, embeddings_batch, k)
