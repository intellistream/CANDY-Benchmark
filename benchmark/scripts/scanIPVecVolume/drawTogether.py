...
def main():
    ...
    commonBasePath = os.path.abspath(os.path.join(os.getcwd(), "../..")) + "/results/scanIPVecVolume/"

    figPath = os.path.abspath(os.path.join(os.getcwd(), "../..")) + "/figures/scanIPVecVolume"
    ...
    groupLine.DrawFigureYLog(periodAll, incrementalBuildAll / 1000,
                             methodTags,
                             "#Ingested Vectors", r'95% Latency of insert (ms)', 0, 1,
                             figPath + "/" + "scanIPVecVolume_lat_INSERT_feedMode",
                             True)
    groupLine.DrawFigureYLog(periodAll, pendingWaitTimeAll / 1000,
                             methodTags,
                             "#Ingested Vectors", r'Pending wait for insert (ms)', 0, 1,
                             figPath + "/" + "scanIPVecVolume_lat_pending_feedMode",
                             True)
    groupLine.DrawFigureYLog(periodAll, incrementalSearchAll / 1000,
                             methodTags,
                             "#Ingested Vectors", r'Latency of search (ms)', 0, 1,
                             figPath + "/" + "scanIPVecVolume_lat_search_feedMode",
                             True)
    groupLine.DrawFigureYLog(periodAll, (incrementalSearchAll + pendingWaitTimeAll) / 1000,
                             methodTags,
                             "#Ingested Vectors", r'Latency of query (ms)', 0, 1,
                             figPath + "/" + "scanIPVecVolume_lat_instant_search",
                             False)
