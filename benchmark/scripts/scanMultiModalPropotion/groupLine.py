...
def DrawViolinFigure(xvalues, yvalues, legend_labels, x_label, y_label, y_min, y_max, filename, allow_legend):
    # 创建画布
    fig, ax = plt.subplots(figsize=(10, 6))

    # 对每一个算法的数据单独处理
    for i in range(len(yvalues)):
        x_data = xvalues[i]  # 当前算法的 x 数据
        y_data = yvalues[i]  # 当前算法的 y 数据
        
        # 对 x 进行分箱
        x_bins = pd.cut(x_data, bins=10, labels=[f'Bin {j}' for j in range(1, 11)])
        
        # 构建 DataFrame 以适应 Seaborn 的输入格式
        data = pd.DataFrame({
            'X': x_bins,
            'Y': y_data
        })
        
        # 使用 sns.violinplot 叠加绘制不同算法的小提琴图
        sns.violinplot(x='X', y='Y', data=data, ax=ax, inner=None)
...
def DrawFigureYLog(xvalues, yvalues, legend_labels, x_label, y_label, y_min, y_max, filename, allow_legend):
    ...
    if allow_legend == True:
        plt.legend(lines,
                   ...
                   loc='upper center',
                   ncol=2,
                   bbox_to_anchor=(-0.6, 1.0), shadow=False,
                   columnspacing=0.1,
                   frameon=True, borderaxespad=0, handlelength=1.2,
                   handletextpad=0.1,
                   labelspacing=0.1)
