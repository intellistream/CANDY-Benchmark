...
def runPeriod(exePath, algoTag, resultPath, configTemplate="config.csv", prefixTagRaw="null"):
    ...
    editConfig(configTemplate, "temp2.csv", "vecDim", prefixTag)
...
def main():
    ...
    commonBasePath = os.path.abspath(os.path.join(os.getcwd(), "../..")) + "/results/scanIPDimensions/"

    figPath = os.path.abspath(os.path.join(os.getcwd(), "../..")) + "/figures/scanIPDimensions"
    ...
    groupLine.DrawFigureYLog(periodAll, incrementalBuildAll / 1000,
                             methodTags,
                             "Dimensions", r'95% Latency of insert (ms)', 0, 1,
                             figPath + "/" + "scanIPDimensions_lat_INSERT_feedMode",
                             True)
    groupLine.DrawFigureYLog(periodAll, pendingWaitTimeAll / 1000,
                             methodTags,
                             "Dimensions", r'Pending wait for insert (ms)', 0, 1,
                             figPath + "/" + "scanIPDimensions_lat_pending_feedMode",
                             True)
    groupLine.DrawFigureYLog(periodAll, incrementalSearchAll / 1000,
                             methodTags,
                             "Dimensions", r'Latency of search (ms)', 0, 1,
                             figPath + "/" + "scanIPDimensions_lat_search_feedMode",
                             True)
    groupLine.DrawFigureYLog(periodAll, (incrementalSearchAll + pendingWaitTimeAll) / 1000,
                             methodTags,
                             "Dimensions", r'Latency of query (ms)', 0, 1,
                             figPath + "/" + "scanIPDimensions_lat_instant_search",
                             False)
    groupLine.DrawFigureYnormal(periodAll, recall,
                                methodTags,
                                "Dimensions", r'Recall@10', 0, 1,
                                figPath + "/" + "scanIPDimensions_recall_feedMode",
                                True)
    groupLine.DrawFigureYLog(periodAll, 1/(incrementalSearchAll / 1e6),
                                methodTags,
                                "Dimensions", r'Queries Per Second', 0, 1,
                                figPath + "/" + "scanIPDimensions_qps",
                                False)
