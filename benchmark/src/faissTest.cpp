...
int main(int argc, char **argv) {
  ...
  if (inMap->fromCArg(argc, argv) == false) {
    if (argc >= 2) {
      std::string fileName = "";
      fileName += argv[1];
      if (inMap->fromFile(fileName)) { std::cout << "load from file " + fileName << endl; }
    }
  }
  INTELLI::MemoryTracker mt;
  INTELLI::MemoryTracker::setActiveInstance(&mt);
  database_size = inMap->tryI64("vecVolume", 1000);
  ...
  int64_t vector_dim = inMap->tryI64("vecDim", 768);
  int64_t k = inMap->tryI64("k", 5);
  std::mt19937_64 gen;
  gen = std::mt19937_64(114514);
  CANDY::DataLoaderTable dt;
  auto dl = dt.findDataLoader("random");
  dl->setConfig(inMap);
  float *database_data = dl->getData().contiguous().data_ptr<float>();
  float *query_data = dl->getQuery().contiguous().data_ptr<float>();
  ...
  faiss::IndexFlat indexInc(vector_dim);
  // Calculate the number of chunks needed
  size_t numChunks = (database_size + chunkSize - 1) / chunkSize;
  std::cout
      << "incremental Raw w/o retrain, step size =" + std::to_string(pqFeedStep) + ",using " + std::to_string(numChunks)
          + " steps" << std::endl;
  if (fullRebuild) {
  ...
  for (int i = 0; i < query_size; ++i) {
    for (int j = 0; j < k; ++j) {
      ...
      if (std::find(index_groundTruth.begin() + i * k, index_groundTruth.begin() + (i + 1) * k, index_pq[i * k + j])
          != index_groundTruth.begin() + (i + 1) * k) {
        truePositives++;
      } else {
        falseNegatives++;
      }
  ...
  cfg.edit("cpuAvg", (double) mt.getAvgCpu());
  cfg.edit("cpuMax", (double) mt.getMaxCpu());
