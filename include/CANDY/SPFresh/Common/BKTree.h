...
#include "CANDY/SPFresh/Common/cuda/Kmeans.hxx"
...
#pragma omp parallel for num_threads(args._T) shared(data, indices) reduction(+:currDist)
        ...
        float TryClustering(const Dataset<T>& data,
            ...
            for (int iter = 0; iter < 100; iter++) {
                ...
                args.ClearCounts();
                args.ClearDists(-MaxDist);
                currDist = KmeansAssign<T, R>(data, indices, first, batchEnd, args, true, std::min(adjustedLambda, originalLambda));
        ...
        public:
                ...
                bool dynamicK = false, IAbortOperation* abort = nullptr)
            {
                ...
                {std::mt19937 rg;
                    ...
                    //printf("Start to build BKTree %d\n", i + 1);
