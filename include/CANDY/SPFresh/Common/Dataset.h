...
#include "CANDY/SPFresh/Helper/Logging.h"
#include <stdexcept>
#include <sstream>
        ...
        public:
            ...
            ErrorCode AddBatch(const T* pData, SizeType num)
            {
                ...
                while (written < num) {
                    ...
                    SizeType toWrite = std::min(rowsInBlock + 1 - curBlockPos, num - written);
            ...
            ErrorCode AddBatch(SizeType num)
            {
                ...
                while (written < num) {
                    ...
                    written += std::min(rowsInBlock + 1 - ((incRows + written) & rowsInBlock), num - written);
                }
                incRows += written;
                return ErrorCode::Success;
            }
