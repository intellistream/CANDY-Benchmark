#pragma once
#ifndef CANDY_INCLUDE_ALGORITHMS_SONG_CUCKOOFILTER_HPP
#define CANDY_INCLUDE_ALGORITHMS_SONG_CUCKOOFILTER_HPP
#define bucketSize 4
...
class CuckooFilter {
 public:
  ...
  __device__ bucket_t hash2fp(idx_t x) { return (x % BUCKET_T_MOD) + 1; }
  ...
  __device__ bool test(idx_t x) {
    ...
    return getFingerprintIndex(buckets[i1], fp) > -1 ||
           getFingerprintIndex(buckets[i2], fp) > -1;
  }

  __device__ int randi(int i1, int i2) {
    return ((i1 * (i2 >> 5)) ^ i2 ^ buckets[(i1 + i2) / 2][0]) % 2 == 0 ? i1
                                                                        : i2;
  }

  // Insert inserts data into the counter and returns true upon success
  __device__ bool Insert(idx_t x) {
    int i1, i2;
    bucket_t fp;
    getIndicesFingerprint(x, i1, i2, fp);
  ...
  __device__ bool insert(bucket_t x, int i) {
  ...
  __device__ bool reinsert(bucket_t x, int i) {
  ...
  __device__ bool internal_del(bucket_t x, int i) {
  ...
  __device__ bool bucket_insert(bucket_t* bucket, bucket_t x) {
  ...
  __device__ bool bucket_delete(bucket_t* bucket, bucket_t x) {
    for (int i = 0; i < bucketSize; ++i) {
      if (bucket[i] == x) {
        bucket[i] = 0;
        return true;
  ...
  __device__ int getFingerprintIndex(bucket_t* bucket, bucket_t x) {
    for (int i = 0; i < bucketSize; ++i) {
      if (bucket[i] == x) {
        return i;
      }
  ...
  __device__ int inline hash(idx_t x) {
    x ^= x >> 33;
