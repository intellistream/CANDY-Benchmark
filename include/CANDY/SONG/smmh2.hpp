...
#ifndef CANDY_INCLUDE_ALGORITHMS_SONG_SMMH2_HPP
#define CANDY_INCLUDE_ALGORITHMS_SONG_SMMH2_HPP
...
__device__ int inline parent(int x) {
  return ((x - 1) >> 1);
}

__device__ int inline grandparent(int x) {
  return ((x - 3) >> 2);
}

__device__ int inline leftchild(int x) {
  return ((x << 1) + 1);
}

__device__ int inline rightchild(int x) {
  return ((x << 1) + 2);
}

__device__ bool inline is_leaf(unsigned int x, int max_size) {
