...
#ifndef CANDY_INCLUDE_ALGORITHMS_SONG_DATA_HPP
#define CANDY_INCLUDE_ALGORITHMS_SONG_DATA_HPP
...
class Data {
 ...
 public:
  ...
  value_t* get(idx_t idx) const { return data.get() + idx * dim; }

  void del(idx_t idx) { memset(get(idx), 0, sizeof(value_t) * dim); }
...
#ifdef __ENABLE_HASH
  dist_t inline test_hamming(int a, int b) {
  ...
  template <class T>
  dist_t inline bit_hamming_distance(idx_t a, T& v) const {
    auto pa = get(a);
    dist_t ret = 0;
    for (int i = 0; i < dim; ++i) {
      auto diff = (*(pa + i)) ^ (v[i]);
  ...
  std::vector<value_t> organize_point(
      const std::vector<std::pair<int, value_t>>& v) {
...
#ifdef __ENABLE_HASH
  void inline add(idx_t idx, std::vector<bool>& value) {
...
template <>
dist_t inline Data::l2_distance(idx_t a, idx_t& b) const {
  auto pa = get(a), pb = get(b);
  dist_t ret = 0;
  for (int i = 0; i < dim; ++i) {
    auto diff = *(pa + i) - *(pb + i);
...
template <>
dist_t inline Data::negative_inner_prod_distance(idx_t a, idx_t& b) const {
  auto pa = get(a), pb = get(b);
  dist_t ret = 0;
  for (int i = 0; i < dim; ++i) {
    ret -= (*(pa + i)) * (*(pb + i));
...
template <>
dist_t inline Data::negative_cosine_distance(idx_t a, idx_t& b) const {
  auto pa = get(a), pb = get(b);
  dist_t ret = 0;
  value_t lena = 0, lenv = 0;
  for (int i = 0; i < dim; ++i) {
...
#ifdef __ENABLE_HASH
template <>
dist_t inline Data::bit_hamming_distance(idx_t a, idx_t& b) const {
  auto pa = get(a), pb = get(b);
  dist_t ret = 0;
  for (int i = 0; i < dim; ++i) {
    auto diff = (*(pa + i)) ^ (*(pb + i));
