...
#ifndef CANDY_INCLUDE_ALGORITHMS_SONG_FIXHASH_HPP
#define CANDY_INCLUDE_ALGORITHMS_SONG_FIXHASH_HPP

namespace SONG_KERNEL {
template <typename T, const int max_size, const int EMPTY = -1,
...
struct FixHash {
  ...
  __device__ short hash(int h, idx_t x) {
    return (x ^ (x >> 16) * random_number[h << 1] ^
            random_number[(h << 1) + 1]) %
           max_size;
