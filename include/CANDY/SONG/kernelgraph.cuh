#ifndef CANDY_INCLUDE_ALGORITHMS_SONG_KERNELGRAPH_CUH
#define CANDY_INCLUDE_ALGORITHMS_SONG_KERNELGRAPH_CUH
...
#include<CANDY/SONG/warp_astar_accelerator.cuh>
...
public:
    ...
    void add_vertex_new(idx_t vertex_id,std::vector<std::pair<int,value_t>>& point) {
        ...
        int num_neighbors = degree < neighbor[0].size() ? degree : neighbor[0].size();
        auto offset = vertex_id << vertex_offset_shift;
        edges[offset] = num_neighbors;
        // TODO:
        // it is possible to save this space --- edges[offset]
    ...
    void delete_vertex(const std::vector<std::pair<int,value_t>>& point) {
        // std::vector<std::vector<idx_t>> idx_points;
        // std::vector<std::vector<idx_t>> points(1, point);
        // search_top_k_batch(points, 1, idx_points);
        // int idx_point = idx_points[0][0];
        // auto offset = idx_point << vertex_offset_shift;
        // memset(&edges[offset], 0, sizeof(idx_t) * degree);
        // memset(&edge_dist[offset], 0, sizeof(dist_t) * degree);
        // data->del(idx_point);
    }

    void revise_vertex(const std::vector<std::pair<int,value_t>>& point, const std::vector<std::pair<int,value_t>>& new_point) {
        // std::vector<std::vector<idx_t>> idx_points;
        // std::vector<std::vector<idx_t>> idx_new_points;
        // std::vector<std::vector<idx_t>> points(1, point);
        // std::vector<std::vector<idx_t>> new_points(1, new_point);
        // search_top_k_batch(points, 1, idx_points);
        // search_top_k_batch(new_points, 1, idx_new_points);
        // int idx_point = idx_points[0][0];
        // int idx_new_point = idx_new_points[0][0];
        // data->revise(idx_point, new_point);
    }
