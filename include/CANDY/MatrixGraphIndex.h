/*! \file MatrixGraphIndex.h*/
//
// Created by tony on 04/01/24.
//

#ifndef CANDY_INCLUDE_CANDY_MatrixGraphIndex_H_
#define CANDY_INCLUDE_CANDY_MatrixGraphIndex_H_
 ...
 * @{
 */
class MatrixGraphIndex;
/**
 * @class MatrixGraphIndex CANDY/MatrixGraphIndex.h
 ...
 */
class MatrixGraphIndex : public FlatIndex {
 protected:
   ...
   */
  torch::Tensor (*distanceFunc)(torch::Tensor &db, torch::Tensor &query, int64_t cudaDev, MatrixGraphIndex *idx);
  /**
   ...
   */
  static torch::Tensor distanceIP(torch::Tensor &db, torch::Tensor &query, int64_t cudaDev, MatrixGraphIndex *idx);
  /**
   ...
   * @return The distance tensor, must sized [q*n], will in GPU if cuda is valid
   */
  static torch::Tensor distanceL2(torch::Tensor &db, torch::Tensor &query, int64_t cudaDev, MatrixGraphIndex *idx);
 ...
 * @typedef MatrixGraphIndexPtr
 * @brief The class to describe a shared pointer to @ref  MatrixGraphIndex

 */
typedef std::shared_ptr<class CANDY::MatrixGraphIndex> MatrixGraphIndexPtr;
/**
 * @ingroup  CANDY_lib_bottom
 * @def newMatrixGraphIndex
 * @brief (Macro) To creat a new @ref  MatrixGraphIndex shared pointer.
 */
#define newMatrixGraphIndex std::make_shared<CANDY::MatrixGraphIndex>
}
/**
 * @}
 */
